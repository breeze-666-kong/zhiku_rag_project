...
def step_3_reciprocal_rank_fusion(source_with_weight,top_k:int =5):
    ...
    #2.记录我的chunk片段
    chunk_dict= {}  #chunk_id -> key || chunk {}  不重复
    ...
    for source, weight in source_with_weight:
        ...
        for rank, chunk in enumerate( source,start=1):
            # chunk={id: 实体的主键,distance:得分 0.8,entity:{chunk_id,content,title..}}
            #获取chunk_id
            chunk_id=chunk.get("id") or chunk.get("entity").get("chunk_id")
            #计算得分 rrf权重版本公式=1/(k + rank) * weight,需要把原数据也加进去
...
def node_rrf(state):
    ...
    #3.rrf算法进行排序处理倒叙
    rrf_response= step_3_reciprocal_rank_fusion(soure_with_score)
